...
<body>
	...
	<div class="secenek">
			...
			<?php
				...
				echo "<ul>
						...
						<li class=active><a href='gidilen_sinema.php'>Sinema</li>	
	...
	<div class="tum_liste">		
			...
			<?php	
				...
				$rowSet1 = mysqli_query($mysqli, $sql1 = "SELECT * FROM gidilen_sinema WHERE email='$email';"); 
					...
					echo    "<table><thead><th>Film Adı</th><th>Vizyon Tarihi</th><th>Film Türü</th></thead>";
						while($kayit = mysqli_fetch_assoc($rowSet1)){
								echo "<tr><td>{$kayit['name']}</td>  	
									  <td>{$kayit['date']}</td>
		                              <td>{$kayit['kind']}</td>
		                              <td><a href='main.php?&email={$email}&sinemaadi={$kayit['name']}&sinematarihi={$kayit['date']}&islem=sinemasil'>Listeden Sil</a></td></tr>";
					}
